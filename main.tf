resource "vsphere_virtual_machine" "k8s_masters_vm"  {
  ...
  disk {
    label = "data_disk1"
    size  = var.add_disks.diskm1.sizegb
    unit_number = 1
    thin_provisioned = true
  }
  disk {
    label = "data_disk2"
    size  = var.add_disks.diskm2.sizegb
    unit_number = 2
    thin_provisioned = true
  ...
  extra_config = {
     ...
     "guestinfo.metadata" = base64encode(replace((replace((data.template_file.metadata.rendered), "vm_ip_address",var.vms.masters.ip_pool[count.index])), "os_host_name", "${var.vms.masters.pref}-0${count.index + 1}"))
...
resource "vsphere_virtual_machine" "k8s_workers_vm"  {
  ...
  extra_config = {
     ...
     "guestinfo.userdata.encoding" = "base64"
     "guestinfo.metadata" = base64encode(replace((replace((data.template_file.metadata.rendered), "vm_ip_address",var.vms.workers.ip_pool[count.index])), "os_host_name", "${var.vms.workers.pref}-0${count.index + 1}"))
     "guestinfo.metadata.encoding" = "base64"
